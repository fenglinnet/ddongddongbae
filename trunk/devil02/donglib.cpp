...
//¸¶¿ì½º ¹öÆ°
BOOL LeftDown(int delay)
{
	static int left_click_delay;
	...
	if(LButton)
	{
		...
		{
			++left_click_delay;
			if(left_click_delay >= delay)left_click=FALSE;
		}
	}
...
BOOL RightDown(int delay)
{
	static int right_click_delay;
	...
	if(RButton)
	{
		...
		{
			++right_click_delay;
			if(right_click_delay >= delay)left_click=FALSE;
		}
	}
...
{
	JPictureInfo pi;
	if(jdd->GetPictureInfo(pic_dlg, &pi))jdd->DeleteSurface(pic_dlg);	//ÀÌ¹Ì ±×¸²ÀÌ Á¸ÀçÇÏ¸é Áö¿ì±â
...
{
	if(dlgbox == NULL)return;	//´ëÈ­Ã¢ÀÌ ¾øÀ¸¸é µ¹¾Æ°¡±â
...
CCommand::CCommand(char* source[], int max)
{
	Init(source,max);
}
...
{
	frame=vframe;
	if(type==ani_exchange)	//¿Ô´Ù°¬´Ù ¹İº¹ ¾Ö´Ï¸ŞÀÌ¼Ç
	{
		int frame2=abs((frame_max/2)-frame);
		SetRect(&ani_rect, x_size*frame2, y_point, x_size*(frame2+1), y_point+y_size);
	}
...
BOOL MainInitialize(char* window_name, HINSTANCE hInstance, BOOL use_keyboard, BOOL use_mouse, bool window_mode)
{
	...
	{
		if(!window_mode)jdd->DrawPicture(backbuffer, "CommonLoading", 0, 0, NULL);
		jdd->Render();
